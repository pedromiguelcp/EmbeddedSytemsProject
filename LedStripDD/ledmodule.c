...
static const int RedLed = 16;
static const int GreenLed = 20;
static const int BlueLed = 21;
...
ssize_t led_device_write(struct file *pfile, const char __user *pbuff, size_t len, loff_t *off) { //tirar o static
	...
	SetGPIOOutputValue(pdev, RedLed, colour & 0x01);
...
static int __init ledModule_init(void) {
	...
	SetGPIOFunction(s_pGpioRegisters, RedLed, 0b001); //Output
	SetGPIOFunction(s_pGpioRegisters, GreenLed, 0b001); //Output
	SetGPIOFunction(s_pGpioRegisters, BlueLed, 0b001); //Output
...
static void __exit ledModule_exit(void) {
	...
	SetGPIOFunction(s_pGpioRegisters, RedLed, 0); //Configure the pin as input
	SetGPIOFunction(s_pGpioRegisters, GreenLed, 0); //Configure the pin as input
	SetGPIOFunction(s_pGpioRegisters, BlueLed, 0); //Configure the pin as input
